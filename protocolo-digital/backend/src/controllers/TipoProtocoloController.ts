...
import { TipoProtocoloService } from '../services/TipoProtocoloService';

const router = express.Router() as express.Router;
const service = new TipoProtocoloService();
...
router.put('/:id', async (req: Request, res: Response) => {
    const tipo = await service.atualizar(parseInt(req.params.id, 10), req.body);
    res.json(tipo);
});
